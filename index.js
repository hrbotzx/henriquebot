	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `ADEUS @${num.split('@')[0]}VOLTE SEMPRE👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
		...
		try {
			...
			mess = {
				...
				only: {
					...
					ownerB: 'ESTE COMANDO SÓ PODE SER USADO PELO MEU PAI(DONO DO BOT)!',
					admin: 'ESTE COMANDO SÓ PODE SER USADO PELOS ADMINISTRADORES DO GRUPO!',
			...
			switch(command) {
				...
				case 'Limpar tudo':
					...
					reply('Bate papo excluído com sucesso :)')
					break
				case 'bc':
					if (!isOwner) return reply('Quem é você?')
				...
				case 'simih':
					...
					if (Number(args[0]) === 1) {
						...
						reply('Ativando o modo simi com sucesso neste grupo.')
				...
				case 'welcome':
					...
					if (args.length < 1) return reply('Hmmmm')
					if (Number(args[0]) === 1) {
						if (isWelkom) return reply('Ja ativou.')
						welkom.push(from)
				...
				case 'clone':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (args.length < 1) return reply('Selecione a tag alvo que você deseja clonar.')
